...
def _build_frames(
        scenario: str, num_frames: int, height: int, width: int,
...
def _run_once_per_frame_timing(
...
def _format_single_summary(
        args: argparse.Namespace, stats: dict[str, float]) -> list[str]:
    lines = [
...
def _format_repeated_summary(
        args: argparse.Namespace, runs: list[dict[str, float]]) -> list[str]:
...
def _run_single(
        args: argparse.Namespace, frames: list[np.ndarray]) -> tuple[list[str], list[str]]:
